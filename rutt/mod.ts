    ...
    try {
      for (const { pattern, methods } of internalRoutes) {
        ...
        if (pattern instanceof URLPattern) {
          const newPattern = new URLPattern({ pathname: basePath + pattern.pathname });
          res = newPattern.exec({ pathname: pathname });
          groups = res?.pathname.groups ?? {};
        } else {
          const newPattern = new RegExp((basePath.length) ? "^" + basePath + "*" + pattern.source : pattern.source, pattern.flags);
          res = newPattern.exec(pathname);
          groups = res?.groups ?? {};
        }
